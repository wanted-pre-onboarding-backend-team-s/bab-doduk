...
public interface RestaurantReviewRepository extends JpaRepository<RestaurantReview, Long> {
    ...
    @Query(value = "select new com.wanted.babdoduk.restaurant.domain.review.dto.ResultGetRestaurantReviewStatDto ("
            + "r.restaurantId,"
            + " cast(count(r) as int),"
            + " truncate(sum(cast(r.score as double)) / count(r), 2))"
            + " from RestaurantReview r"
            + " where r.restaurantId = :restaurantId")
    ResultGetRestaurantReviewStatDto getReviewCountAndScoreAverage(@Param("restaurantId") Long restaurantId);
